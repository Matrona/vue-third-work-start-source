    ...
    <section class="task-card__wrapper">
      ...
      <div class="task-card__block">
        <div class="task-card__row">
          <!--Наименование задачи-->
          <h1 class="task-card__name task-card__name--min">{{ task ? task.title : '' }}</h1>
          <!--Кнопка редактирования задачи-->
          <a v-if="authStore.getUserAttribute('isAdmin')" class="task-card__edit" @click="router.push({
            name: 'TaskEdit',
      ...
      <!--Комментарии-->
      <task-card-view-comments v-if="task" class="task-card__comments" :comments="task.comments || []" :task-id="task.id"
...
// Мы намеренно используем нестрогое сравнение ==, чтобы не добавлять дополнительные проверки, так как id может быть как строкой "4", так и числом 4.
