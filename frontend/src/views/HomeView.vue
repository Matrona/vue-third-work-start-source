<template>
  <main class="content">
    <section class="desk">
        ...
        <div class="desk__filters">
          <div class="desk__user-filter">
            ...
            <ul class="user-filter">
              <li v-for="user in usersStore.users" :key="user.id" :title="user.name" class="user-filter__item"
      ...
      <!--      Колонки и задачи-->
      <div class="desk__columns" v-if="columnsStore.columns.length">

        <!--        Показываем колонки-->
        <desk-column v-for="column in columnsStore.columns" :key="column.id" :column="column"
          @update="columnsStore.updateColumn" @delete="columnsStore.deleteColumn" />
      </div>
