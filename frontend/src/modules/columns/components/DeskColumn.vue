<template>
  ...
  <app-drop class="column" @drop="moveTask">
    <h2 class="column__name">
      ...
      <input v-else ref="columnTitle" v-model="state.columnTitle" type="text" class="column__input" name="column_title" @blur="updateInput" />
      ...
      <app-icon v-if="!state.isInputShowed && !columnTasks.length" class="icon--trash" @click="$emit('delete', column.id)" />
    </h2>
...
<script setup>
  ...
  const columnTasks = computed(() => {
    return tasksStore.filteredTasks.filter((task) => task.columnId === props.column.id).sort((a, b) => a.sortOrder - b.sortOrder);
  });
